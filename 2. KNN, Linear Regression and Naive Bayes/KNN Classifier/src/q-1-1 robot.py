...
# ### Reading the Data Set Robot1

# In[ ]:


dataSet = pd.read_csv("./../input_data/RobotDataset/Robot1", header=None, delimiter = "\s+")
randomDataSet = dataSet.sample(frac=1).reset_index(drop=True)
Class = "class"
columns = [Class,'A', 'B','C','D','E', 'F', 'G']
...
# ### Validating the Robot1 data set with default euclidian Distance
...
# ### Validating the Robot1 data set with Manhattan Distance
...
# ### Validating the Robot1 data set with Minkowski Distance
...
# ### Reading the Data Set Robot2

# In[ ]:


dataSet = pd.read_csv("./../input_data/RobotDataset/Robot2", header=None, delimiter = "\s+")
randomDataSet = dataSet.sample(frac=1).reset_index(drop=True)
Class = "class"
columns = [Class,'A', 'B','C','D','E', 'F', 'G']
randomDataSet.columns = columns
...
# ### Validating the Robot2 data set with default Euclidian Distance
...
# ### Validating the Robot2 data set with Manhattan Distance
...
# ### Validating the Robot2 data set with Minkowski Distance
...
def test(arr, K):
    ...
    if length == 3:
        dataSetPath = arr[1]
        testSetPath = arr[2]
        dataSet = pd.read_csv(dataSetPath , header=None, delimiter = "\s+")
        testingData = pd.read_csv(testSetPath, header=None, delimiter = "\s+")
        ...
        testingData.to_csv("./../output_data/q-1-1_robot_output.csv",index = False)
    return testingData
