...
def three_layer_convnet(x, params):
    ...
    ############################################################################
    padds = tf.constant([[0,0], [2,2], [2,2], [0,0]])
    x = tf.pad(x, padds, mode ='CONSTANT')
    con_out1 = tf.nn.conv2d(input = x, filter = conv_w1, strides = [1,1,1,1], padding = "VALID") + conv_b1
    relu_out1 = tf.nn.relu(con_out1)
    
    padds = tf.constant([[0,0], [1,1], [1,1], [0,0]])
    con_out1 = tf.pad(con_out1, padds, mode ='CONSTANT')
    con_out2 = tf.nn.conv2d(input = con_out1, filter = conv_w2, strides = [1,1,1,1], padding = "VALID") + conv_b2
    relu_out2 = tf.nn.relu(con_out2)
    
    flat_out2 = flatten(relu_out2)
    scores = tf.matmul(flat_out2, fc_w) + fc_b
    ############################################################################
    #                              END OF YOUR CODE                            #
    ############################################################################
    return scores
...
def three_layer_convnet_init():
    ...
    P = 1
    H1, W1, D1 = 32, 32, 16
    H2, W2, D2 = int((H1 + 2*P - F)/1 + 1), int((W1 + 2*P - F)/1 + 1), D1
    
    fc_w = tf.Variable(kaiming_normal([H2*W2*D2,10]))
...
class ThreeLayerConvNet(tf.keras.Model):
    def __init__(self, channel_1, channel_2, num_classes):
        ...
        self.conv_Layer1 = tf.layers.Conv2D(channel_1, [5,5], [1,1], padding='VALID', kernel_initializer = initializer, activation = tf.nn.relu)
        self.conv_Layer2 = tf.layers.Conv2D(channel_2, [3,3], [1,1], padding='VALID', kernel_initializer = initializer, activation = tf.nn.relu)
        self.fullyConnectedLayer = tf.layers.Dense(num_classes, kernel_initializer = initializer)
    ...
    def call(self, x, training=None):
        ...
        scores = self.fullyConnectedLayer(x)
...
def model_init_fn(inputs, is_training):
    ...
    layers = [
            ...
            tf.keras.layers.ZeroPadding2D(((2,2),(2,2)),data_format="channels_last"),
            tf.keras.layers.Conv2D(channel_1, [5,5], [1,1], padding='valid',
                                   activation=tf.nn.relu,
                                   kernel_initializer=initializer
                                  ),
            tf.keras.layers.ZeroPadding2D(((1,1),(1,1)),data_format="channels_last"),
...
def model_init_fn(inputs, is_training):
    ...
    layers = [
            tf.keras.layers.Conv2D(channel_1, [3,3], [1,1], padding='same',
                                   activation=tf.nn.relu,
                                   kernel_initializer=initializer
                                  ),
            ...
            tf.keras.layers.Conv2D(channel_3, [3,3], [1,1], padding='valid',
                                   activation=tf.nn.relu, 
                                   kernel_initializer=initializer
                                  ),        
            tf.keras.layers.BatchNormalization(),
...
# # Solution
# **Created 3 layers of convolution and 3 layers of fully connected dense network using keras sequential API.**
# 
# **Steps as follows:**
# 
# 1. conv layer1 = 32 filters of 3x3 with strid 1, activated with ReLU.
# 2. Batch Normalization
# 3. Max Pooling with 2x2 filter
# 4. conv layer2 = 64 filters of 3x3 with strid 1, activated with ReLU.
# 5. Batch Normalization
# 6. Max Pooling with 2x2 filter
# 7. conv layer3 = 128 filters of 3x3 with strid 1, activated with ReLU.
# 8. Batch Normalization
# 9. Average Pooling with 5x5 filter
# 10. Fatten 
# 11. 2 Fully Connected dense layer with size 50
# 12. Output layer  with size 10
# 
